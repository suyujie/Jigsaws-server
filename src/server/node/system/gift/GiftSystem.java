...
public final class GiftSystem extends AbstractSystem {
	...
	private GiftBag readGiftBagFromDB(Player player) throws SQLException {
		...
		if (map != null) {
			...
			Map<Long, GiftWear> giftWears = (Map<Long, GiftWear>) SerializerJson.deSerializeMap(json,
					new TypeReference<HashMap<Long, GiftWear>>() {
					});
	...
	// 可否送耐久
	public boolean canGiveWear(Player giver, Player accepter, GiftBag accepterGiftBag, boolean checkStatus)
			throws SQLException {
		...
		if (accepterGiftBag.getGiftWears() != null && accepterGiftBag.getGiftWears().isEmpty() == false) {
			for (GiftWear gift : accepterGiftBag.getGiftWears().values()) {
				...
				if (gift instanceof GiftWear) {
					// 只检查 时间是否允许
					if (gift.getGiverId().longValue() == giver.getId().longValue()
							&& Clock.currentTimeSecond() - gift.getGiveTime() < Content.giftTimePeriod) {// 之前赠送过,而且还在时间限定内
						canGive = false;
						break;
					}
	...
	public SystemResult acceptGift(Player player, Long giftId) throws SQLException {
		...
		while (iter.hasNext()) {
			...
			if (gift.status == GiftStatus.Accept
					&& Clock.currentTimeSecond() - gift.getGiveTime() > Content.giftTimePeriod) {
				iter.remove();
			}
		}
