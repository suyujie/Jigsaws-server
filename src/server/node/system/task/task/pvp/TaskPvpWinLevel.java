...
public class TaskPvpWinLevel extends AbstractTask implements ITask {

	private static final long serialVersionUID = -4247361794653238260L;

	public TaskPvpWinLevel() {
		super(TaskType.PvpWinLevel);
	}

	public void doTask(Player player, int defenderLevel) {

		if (completedNum < defenderLevel) {
			completedNum = defenderLevel;
			for (int i = 0; i < completed.length; i++) {
				if (completed[i] == 0) {//没完成的
					TaskMaking taskMaking = TaskLoadData.getInstance().getTaskMaking(taskType.asCode());
					if (defenderLevel >= taskMaking.getNeedNumTable()[i]) {//完成了
						completed[i] = 1;
					}
				}
			}
