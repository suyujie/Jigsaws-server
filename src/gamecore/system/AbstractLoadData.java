...
public abstract class AbstractLoadData {
	...
	public String getNewXmlName(String xmlName) {
		...
		if (xmlNameList.size() == 1) {//只有一个的时候
			...
			if (version == 0) {
				return new StringBuffer(StorageManager.getInstance().dataPath).append(xmlName).append(".xml").toString();
			} else {
				return new StringBuffer(StorageManager.getInstance().dataPath).append(xmlName).append("_").append(version).append(".xml").toString();
			}
		}
