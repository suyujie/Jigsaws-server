package common.qcloud.cosapi.api;
...
import common.qcloud.cosapi.common.HMACSHA1;
import common.qcloud.cosapi.common.Request;
import common.qcloud.cosapi.common.Sign;
...
public class CosCloud {
	...
	public String updateFile(String bucketName, String remotePath, String bizAttribute) throws Exception {
		...
		String sign = Sign.appSignatureOnce(appId, secretId, secretKey,
	...
	public String deleteFile(String bucketName, String remotePath) throws Exception {
		if (remotePath.equals("/")) {
			throw new Exception(
					"can not delete bucket using aip! go to http://console.qcloud.com/cos to operate bucket");
		}
		String url = COSAPI_CGI_URL + appId + "/" + bucketName + encodeRemotePath(remotePath);
		HashMap<String, Object> data = new HashMap<String, Object>();
		data.put("op", "delete");
	 ...
	 *            拉取的总数
	 * @param context
	 *            透传字段，查看第一页，则传空字符串。若需要翻页，需要将前一页返回值中的context透传到参数中。order用于指定翻页顺序
	 *            。若order填0，则从当前页正序/往下翻页；若order填1，则从当前页倒序/往上翻页。
	 * @param order
	 *            默认正序(=0), 填1为反序
	 * @param pattern
	 *            拉取模式:只是文件，只是文件夹，全部
	 ...
	 * @throws Exception
	 */
	public String getFolderList(String bucketName, String remotePath, int num, String context, int order,
			FolderPattern pattern) throws Exception {
		remotePath = standardizationRemotePath(remotePath);
	 ...
	 * @throws Exception
	 */
	public String getFolderList(String bucketName, String remotePath, String prefix, int num, String context, int order,
			FolderPattern pattern) throws Exception {
		remotePath = standardizationRemotePath(remotePath);
		String url = COSAPI_CGI_URL + appId + "/" + bucketName + encodeRemotePath(remotePath)
	 ...
	 * @throws Exception
	 */
	public String sliceUploadFileFollowStep(String bucketName, String remotePath, String localPath, String sessionId,
	 ...
	 * @throws Exception
	 */
	public String sliceUploadFile(String bucketName, String remotePath, String localPath, int sliceSize)
			throws Exception {
